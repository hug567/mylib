...
#make -C ../tools/testing/selftests O=`pwd`/tools/testing/selftests ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- V=1 -j1 #$CPU_THREAD

# qemu启动命令
END_TIME=$(date +%s)
log_info "time spend: $(time_diff ${END_TIME} ${START_TIME})"
log_info "Run kernel: $MYLIB/linux/shell/aarch64/qemu_run_Image.sh"