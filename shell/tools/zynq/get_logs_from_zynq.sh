...
function usage() {
    echo "Usage: $0                  get logs from zynq"
}
...
function is_zynq_alive() {
}

function get_logs_from_zynq() {
}

function main() {
    local logs_file="logs.tar.gz"

    if [ -f ${logs_file} ]; then
        echo "${logs_file} is already existed in current dir"
        exit 1
    fi
